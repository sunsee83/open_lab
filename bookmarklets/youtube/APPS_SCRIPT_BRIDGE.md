...
UI가 준비된 뒤에는 최초 `YTDL_UI_READY`를 보낸 window와 origin을 현재 실행의 UI 채널로 고정합니다.
...
pick-file   단일 로컬 파일 선택
pick-dir    로컬 폴더 선택
dir-file    선택 폴더 안 파일 핸들 생성
...
일반 POST 응답은 `YT_GAS_RESPONSE`의 token과 requestId가 일치할 때만 사용합니다.
