...
function makeParent(){const y=C.ytcfg||{};return{document:metaDoc(),location:{href:String(C.href||''),search:String(C.search||''),pathname:String(C.pathname||''),origin:O},fetch:hostFetch,ytcfg:{get:n=>y[n]},ytInitialData:C.initialData||{},open:(url,target)=>rpc('open',{url:String(url||''),target:String(target||'_blank')}),__YTDL_CALL:(a,p)=>rpc('gas',{action:a,payload:p||{}}),__YTDL_WEBAPP_URL:String(C.webapp||''),__YTDL_TOKEN:T,__YTDL_CLOSE:()=>rpc('close',{})}}
